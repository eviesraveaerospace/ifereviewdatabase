...
def _build_article_preview(url: str) -> str:
    ...
    for t in soup.find_all(["script", "noscript", "iframe", "object", "embed"]):
        t.decompose()
    for tag in soup.find_all(True):
        for attr in [k for k in tag.attrs if k.lower().startswith("on")]:
            del tag.attrs[attr]
    # Lazy-loaded images never get their real src without JS — promote data-* attributes.
    for img in soup.find_all("img"):
        for k in ("data-src", "data-lazy-src", "data-original", "data-url"):
            if img.get(k) and not (img.get("src") or "").startswith("http"):
                img["src"] = img[k]; break
...
def article_preview():
    ...
    known = any(r.get("url") == url for r in data_manager.data.get("reviews", []))
    if not known or not url.startswith(("http://", "https://")):
        return Response("Unknown article", status=404, mimetype="text/plain")
    hit = _PREVIEW_CACHE.get(url)
    if hit and time.time() - hit[0] < _PREVIEW_TTL:
        html = hit[1]
    else:
        try:
            html = _build_article_preview(url)
        except Exception as e:  # noqa: BLE001
            return Response(f"Preview unavailable: {e}", status=502, mimetype="text/plain")
        _PREVIEW_CACHE[url] = (time.time(), html)
